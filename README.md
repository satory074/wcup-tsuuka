...
- `matches[].goals`（任意）= 得点イベント `{ "minute": 70, "plus": 5(任意), "side": "home"|"away", "player": "選手名"(任意) }` の配列。**最終節（第3節）の分刻みタイムライン**に使う。`side` は得点が入る側（オウンゴールは利益を得た側、`player` は `"アゲルド(OG)"` 形式）。`player` は日本選手=漢字・他=カタカナで列ヘッダに表示。**本数は score と一致必須**（`validate.ts` が検出）。第3節16試合に投入済み。
...
2. モードを選ぶ（どちらも横1表。縦＝順位、列＝時間が右へ流れ、各セル＝その順位の国旗）:
   - **全試合（分刻み）**: 第1〜3節の全6試合を1表に。列＝各ゴール（節ごとに分で統合）、ヘッダは『第n節の帯』＋『時刻行』＋『2レーン（試合①/②）』。得点（⚽得点者・スコア）はその試合のレーンに色分け表示。🟩＝暫定通過圏（上位2の行）、▲▼＝順位変動（国旗の上下）。横スクロールでも先頭の順位列は固定。
