...
ヒーロー機能は **タイムライン（順位バンプチャート）** — 「この時間に（誰が）得点 → この時点ではこの順位」を **1つの表**（縦＝順位 1〜4・右へ時間が流れ、各セルにその順位の**国旗**が入り、時間とともに国旗が上下に動く・列ヘッダに得点選手）で可視化。2モード:
- **全試合（分刻み）**: 第1〜3節の全6試合のゴールを節ごとに分で統合し、ライブ順位・暫定通過圏（上位2）が分刻みで入れ替わる様子（例: グループE で 第3節 70' にコスタリカが2-1とした瞬間、暫定でスペインを追い落とし通過圏入り→ドイツの逆転で元に戻る）。
...
- `matches[].kickoff`（必須）= キックオフ日時 ISO 現地時間 `"2022-11-23T16:00"`。タイムラインの絶対時刻並べ替え＆日時帯表示に使う（最終節の2試合は同一キックオフ＝並列）。
- `matches[].goals`（任意）= 得点イベント `{ "minute": 70, "plus": 5(任意), "side": "home"|"away", "player": "選手名"(任意) }` の配列。**分刻みタイムライン**に使う。`side` は得点が入る側（オウンゴールは利益を得た側、`player` は `"アゲルド(OG)"` 形式）。`player` は日本選手=漢字・他=カタカナで表示。**本数は score と一致必須**（`validate.ts` が検出）。全48試合に投入済み（0-0 は `[]`）。
...
1. グループ（A〜H）を選ぶ。
2. モードを選ぶ（どちらも横1表。縦＝順位、列＝時間が右へ流れ、各セル＝その順位の国旗）:
   - **全試合（分刻み）**: 第1〜3節の全6試合を1表に。並びは**キックオフ日時＋経過分の絶対時刻順**（同時刻開催＝最終節などは分で並列、別時間帯の試合は時系列で前後に）。ヘッダは『第n節の帯』＋『日時帯（キックオフ時間帯ごと、現地時間 `M/D HH:MM`）』＋『時刻行』＋『2レーン（試合①/②）』。得点（⚽得点者・スコア）はその試合のレーンに色分け表示。🟩＝暫定通過圏（上位2の行）、▲▼＝順位変動（国旗の上下）。横スクロールでも先頭の順位列は固定。
   - **大会全体（試合単位）**: 列＝第1→3節の各試合（ヘッダに節・スコア）、各セル＝その試合後の順位の国旗。
