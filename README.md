...
ヒーロー機能は **タイムライン** — 「この時間に（誰が）得点 → この時点ではこの順位」を **1つの表**（左端にチーム名・右へ時間が流れ、各セル＝その時点の順位、列ヘッダに得点選手）で可視化。2モード:
...
1. グループ（A〜H）を選ぶ。
2. モードを選ぶ（どちらも横1表。列＝時間が右へ流れ、行＝チーム、セル＝順位）:
   - **最終節（分刻み）**: 第3節の同時刻2試合のゴールを統合し、列＝キックオフ→各ゴール。列ヘッダに `分・⚽得点者・スコア`。🟩 セル＝暫定通過圏（上位2）、▲▼＝直前からの順位変動。横スクロールでも先頭のチーム名列は固定。
   - **大会全体（試合単位）**: 列＝第1→3節の各試合（ヘッダに節・スコア）、セル＝その試合後の順位。
