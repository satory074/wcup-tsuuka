...
// ---- 8) 大会切替（?cup=2026: 12組・ベスト3位パネル。詳細を明示） ----
{
  ...
  // 日程カルーセル: 2026 は全72試合＋決勝T32＝104カード。組Aの6試合を強調。R32 進行中で M73-M79 が消化。
  ...
  assert(root.querySelectorAll("#schedule .sched-card.is-ko.is-upcoming").length === 25, "8: 2026 KOは M73-M79 消化済み＝25カードが is-upcoming");
  assert(root.querySelectorAll("#schedule .sched-card.is-ko:not(.is-upcoming)").length === 7, "8: 2026 KOで消化済みは M73-M79 の7カード");
  // 3位比較は一覧のみ＝詳細には無い。
  ...
  // R32 進行中: M73-M79 が消化済み＝勝者を強調＋スコア併記（M80 以降は未消化）。
  assert(root.querySelectorAll("#knockout .ko-side.is-winner").length === 7, "8: 2026 KO 消化済みは M73-M79＝勝者ハイライト7");
  assert(root.querySelectorAll("#knockout .ko-score").length === 14, "8: 2026 KO スコア併記は M73-M79 の14枠");
