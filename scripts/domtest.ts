...
// ---- 8) 大会切替（?cup=2026: 12組・ベスト3位パネル。詳細を明示） ----
{
  ...
  // 日程カルーセル: 2026 は全72試合＋決勝T32＝104カード。組Aの6試合を強調。R32 進行中で M73-M82 が消化。
  ...
  assert(root.querySelectorAll("#schedule .sched-card.is-ko.is-upcoming").length === 22, "8: 2026 KOは M73-M82 消化済み＝22カードが is-upcoming");
  assert(root.querySelectorAll("#schedule .sched-card.is-ko:not(.is-upcoming)").length === 10, "8: 2026 KOで消化済みは M73-M82 の10カード");
  // 3位比較は一覧のみ＝詳細には無い。
  ...
  // R32 進行中: M73-M82 が消化済み＝勝者を強調＋スコア併記（M83 以降は未消化）。
  assert(root.querySelectorAll("#knockout .ko-side.is-winner").length === 10, "8: 2026 KO 消化済みは M73-M82＝勝者ハイライト10");
  assert(root.querySelectorAll("#knockout .ko-score").length === 20, "8: 2026 KO スコア併記は M73-M82 の20枠");
