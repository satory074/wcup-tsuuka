...
// ---- 8) 大会切替（?cup=2026: 12組・ベスト3位パネル。詳細を明示） ----
{
  ...
  assert(root.querySelectorAll("#schedule .sched-card.is-ko.is-upcoming").length === 16, "8: 2026 KOは R32 全消化＝R16以降16カードが is-upcoming");
  assert(root.querySelectorAll("#schedule .sched-card.is-ko:not(.is-upcoming)").length === 16, "8: 2026 KOで消化済みは R32 全16カード");
  // 3位比較は一覧のみ＝詳細には無い。
  ...
  assert(root.querySelectorAll("#knockout .ko-side.is-winner").length === 16, "8: 2026 KO R32 全消化＝勝者ハイライト16");
  assert(root.querySelectorAll("#knockout .ko-score").length === 32, "8: 2026 KO スコア併記は R32 全16試合の32枠");
